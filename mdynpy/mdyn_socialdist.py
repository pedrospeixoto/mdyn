...
class socialdist:
    #social distancing class
     def __init__(self, filename, network):
        ...
        (filename_short, ext) = os.path.splitext(filename)
        filename_clean = filename_short + "_clean" + ext

        if os.path.exists(filename_clean):
            print("...from pre-processed file.", filename_clean )
            df = pd.read_csv(filename_clean)
            self.df = df

        else:
            print("...from raw file.", filename )
            ...
            #save to file
            print("...saving processed file for future use.")
