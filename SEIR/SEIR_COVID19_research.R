...
suppressMessages(source("mdyn/SEIR/utils_research.R"))
...
SEIR_covid <- function(cores,par,pos,seed,sample_size,simulate_length,d_max,max_models,error_I,error_D){
  ...
  #Isolation
  par$iso <- list()
  for(d in as.character(day_validate)){
    tmp <- obs %>% filter(date == d) %>% select(city,iso)
    tmp <- tmp[match(par$names,tmp$city),]
    par$iso[[weekdays(ymd(d))]] <- tmp$iso
  }
