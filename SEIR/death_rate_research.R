...
death_rate <- function(teste_D,teste_I,obs,day,drs,par){
  ...
  for(d in unique(drs$DRS)){ #For each DRS
    ...
    dr <- (tmpD$D_drs[tmpD$date == day] - tmpD$D_drs[tmpD$date == ymd(day)-6])/(tmpI$I_drs[tmpI$date == day] - tmpI$I_drs[tmpI$date == ymd(day)-6])#DR around day
  ...
  for(c in C_5){ #For each city with 5+ deaths by day
    tmp <- obs %>% filter(city == c & date >= ymd(day) - 6) #Get data of city is last day of validation
    dr <- (tmp$deaths_corrected[tmp$date == day] - tmp$deaths_corrected[tmp$date == ymd(day)-6])/(tmp$confirmed_corrected[tmp$date == day] - tmp$confirmed_corrected[tmp$date == ymd(day)-6])  #Death rate
    if(dr > 0)
