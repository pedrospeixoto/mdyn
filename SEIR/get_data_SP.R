...
get_data_SP <- function(){
  ...
  for(c in unique(obs$city)){ #For each city
    ...
    for(i in 4:(nrow(tmp)-3)){
      ...
      if(i > 21){ #Estimating recovered
        tmp$recovered[i] <- tmp$recovered[i-1] + tmp$new_infected_cor[i-21] #New corrected confirmed cases 21 days ago are recovered
        tmp$infected[i] <- tmp$confirmed_corrected[i] - tmp$recovered[i] #Delete recovered from infected
      }
      else{ #Correct for first 21 days
        tmp$recovered[i] <- 0
