...
test_model <- function(D,I,teste_D,teste_I,drs,init_validate,end_validate){
  ...
  D <- D %>% filter(DRS != "0")
  dif_D <- max(c(quantile(abs(D$dif)[D$D_drs > 100],0.99),quantile(abs(Dcity),0.99)))
  D <- D %>% filter(D_drs > 100)
  D <- c(D$dif[D$dif < quantile(D$dif,0.99)],Dcity[Dcity < quantile(Dcity,0.99)])
  ...
  I <- I %>% filter(DRS != "0")
  dif_I <- max(c(quantile(abs(I$dif)[I$I_drs > 1000],0.99),quantile(abs(Icity),0.99)))
  I <- I %>% filter(I_drs > 1000)
  I <- c(I$dif[I$dif < quantile(I$dif,0.99)],Icity[Icity < quantile(Icity,0.99)])
