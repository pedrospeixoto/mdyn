...
lift_death <- function(obs,end_validate,par){
  tmp <- obs %>% filter(date >= ymd(end_validate) - 14) #Only end_validate
  lift <- data.frame("city" = NA,"lift" = NA)
  for(c in unique(tmp$city)){
    tmp1 <- tmp %>% filter(city == c)
    deaths <- tmp1$deaths_corrected[tmp1$date == ymd(end_validate)] - tmp1$deaths_corrected[tmp1$date == ymd(end_validate) - 14]
    cases <- tmp1$confirmed_corrected[tmp1$date == ymd(end_validate)] - tmp1$confirmed_corrected[tmp1$date == ymd(end_validate) - 14]
  ...
  lift$lift[lift$lift > 2] <- 2 #Truncate
