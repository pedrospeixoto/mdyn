...
get_data_SP <- function(){
  ...
  for(c in unique(obs$city)){ #For each city
    ...
    for(i in 4:(nrow(tmp)-3)){
      ...
      else{ #Correct for first 15 days
        tmp$recovered[i] <- 0
  ...
  #Isolation
  iso <- readRDS("/home/pedrosp/mdyn/dataR/SP.rds")
  iso$reg_name[iso$reg_name == "BIRITIBA MIRIM"] <- "BIRITIBA-MIRIM"
  ...
  #Isolation of DRS where missing
  drs <- readRDS(file = "mdyn/SEIR/dados/drs.rds")
  drs <- drs %>% select(DRS,Municipio)
  obs <- merge(obs,drs,by.x = "city",by.y = "Municipio",all.x = T,all.y = F)
  obs <- data.table(obs)
  meanRM <- function(x) mean(x,na.rm = T)
  obs$key <- paste(obs$DRS,weekdays(ymd(obs$date)))
  obs <- obs[,mean_pre_DRS := meanRM(mean_pre),by = key]  
  obs$key <- paste(obs$DRS,obs$date)
  obs <- obs[,iso_DRS := meanRM(iso),by = key]  
  obs <- data.frame(obs)
  obs$iso[is.na(obs$iso)] <- obs$iso_DRS[is.na(obs$iso)]
  obs$mean_pre[is.na(obs$mean_pre)] <- obs$mean_pre_DRS[is.na(obs$mean_pre)]
  obs$key <- NULL
  obs$iso_DRS <- NULL  
