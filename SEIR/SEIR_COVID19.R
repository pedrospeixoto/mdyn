...
SEIR_covid <- function(cores,par,pos,seed,sample_size,simulate_length,d_max,max_models,error_I,error_D,process){
  ...
  for(k in 1:sample_size){#For each sampled model
    ...
    parK$beta <- as.vector(apply(bind_rows(lapply(parK$beta,function(x) data.frame(rbind(x)))),2,
                                 function(x) ifelse(median(x) > 0,median(x),min(median(x[x > 0]),max(x),na.rm = T))))
    if(sum(is.na(parK$beta)) > 0){
      is.good[k] <- 0
      prox <- T
    }
    else if(min(parK$beta) < 0){# & max(par$pop[parK$beta < 0]) > 20000){
      is.good[k] <- 0
      prox <- T
    }
    if(prox){
  ...
  dataSim <- store_simulation(predSIM,par,simulate_length,pos,drs,minI,maxI,minD,maxD,end_validate,obs,obs_drs,process)
  saveRDS(object = predSIM,file = paste("/storage/SEIR/",pos,"/simulations_",pos,".rds",sep = "")) #Save results
