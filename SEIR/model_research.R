...
derivatives <- function(t,Y,parK){
  ...
  #States at time t
  E <- parK$iso[[weekdays(parK$day[t])]]*Y[1:parK$sites] #Exposed
  I <- parK$iso[[weekdays(parK$day[t])]]*(Y[(parK$sites + 1):(2*parK$sites)]) #Infected
  Is <- Y[(2*parK$sites + 1):(3*parK$sites)] #Statistics
  R <- (parK$iso[[weekdays(parK$day[t])]]*parK$upI + 1)*Y[(3*parK$sites + 1):(4*parK$sites)] #Recovered
  D <- Y[(4*parK$sites + 1):(5*parK$sites)] #Deaths
  S <- parK$iso[[weekdays(parK$day[t])]]*(parK$pop - Is - D - Y[(3*parK$sites + 1):(4*parK$sites)]) - E - I -  
    parK$iso[[weekdays(parK$day[t])]]*parK$upI*Y[(3*parK$sites + 1):(4*parK$sites)] #Susceptibles
  ...
  N <- parK$iso[[weekdays(parK$day[t])]]*(parK$pop - Is - D - Y[(3*parK$sites + 1):(4*parK$sites)]) + Y[(3*parK$sites + 1):(4*parK$sites)] #Population
  ...
  dY[1:parK$sites] <- -gammaI*E + beta*(S/N)*(s*((mob-diag(diag(mob))) %*% cbind(I)) + I) #E
