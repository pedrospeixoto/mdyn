...
* acertam o número de casos confirmados com um erro máximo de 7\% em pelo menos 90\% das comparações da semana terminando em `r format(dmin,"%d/%m/%Y")`, sendo que uma comparação é formada pela combinação de um DRS com mais de $250$ casos confirmados e um dia.
...
* acertam o número de casos confirmados com um erro máximo de 7\% em pelo menos 90\% das comparações da semana terminando em `r format(dmin,"%d/%m/%Y")`, sendo que uma comparação é formada pela combinação de um Município com mais de $250$ casos confirmados e um dia.
...
* $p_{S}$: `r round(median(param$MedianAssymptomatic),4)` [`r round(min(param$MedianAssymptomatic),4)`;`r round(max(param$MedianAssymptomatic),4)`] proporção de infectados que não são contabilizados nas estatísticas no Estado como um todo
<br>
