...
* acertam o número de mortes com um erro máximo de 4\% em todas os DRSs com mais de $100$ mortes em todos os dias da semana terminando em `r format(dmin,"%d/%m/%Y")`.
* acertam o número de casos confirmados com um erro máximo de 6\% em todas os DRSs com mais de $1,000$ mortes em todos os dias da semana terminando em `r format(dmin,"%d/%m/%Y")`.
* acertam o número de mortes com um erro máximo de 4\% em pelo menos 90\% das comparações da semana terminando em `r format(dmin,"%d/%m/%Y")`, sendo que uma comparação é formada pela combinação de um Município com mais de $100$ mortes confirmadas e um dia.
* acertam o número de casos confirmados com um erro máximo de 6\% em pelo menos 90\% das comparações da semana terminando em `r format(dmin,"%d/%m/%Y")`, sendo que uma comparação é formada pela combinação de um Município com mais de $1,000$ casos confirmados e um dia.
...
* $p_{S}$: `r round(median(param$MedianAssymptomatic),4)` [`r round(min(param$MedianAssymptomatic),4)`;`r round(max(param$MedianAssymptomatic),4)`] proporção de infectados que são contabilizados nas estatísticas no Estado como um todo
<br>
...
<h3 align='justify'> <span style='color: red'> <strong> APENAS PROJEÇÕES PARA CIDADES E DRSs EM QUE FOI TESTADA A ADERÊNCIA DOS MODELOS AOS DADOS (LOCALIDADES COM MAIS DE 1,000 CASOS E 100 MORTES CONFIRMADAS EM `r format(dmin,"%d/%m/%Y")`) SÃO APRESENTADOS NESSE DASHBOARD, EMBORA O MODELO FORNEÇA PREDIÇÕES PARA TODOS OS DRSs E MUNICÍPIOS DO ESTADO. AS ÚNICAS QUANTIDADES QUE DEVEM SER CONDIERADAS NESSE DASHBOARD SÃO AS PREVISÕES DE CASOS E MORTES APRESENTADAS EM LOCAL OPORTUNO ATÉ O DIA `r format(dmax,"%d/%m/%Y")` E A PREVISÃO PARA O PICO DA DOENÇA NAS LOCALIDADES APRESENTADAS. NADA ALÉM DO PICO DA DOENÇA DEVEM SER CONSIDERADAS NAS FIGURAS E IMAGENS.
</h3>
