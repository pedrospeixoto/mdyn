...
params:
  ...
  DRS: 'NULL'
  dmin: 'NULL'
title: "Previsões COVID-19\\newline DRS `r params$DRS`"
...
drs <- params$drs
DRS <- params$DRS
dmin <- min(deaths_city$Date)
peak <- params$peak_city
peak <- peak %>% filter(Municipio %in% drs$Municipio[drs$Regiao == DRS])
data_rel <- ymd(params$input$date)
tmpC <- cases_city %>% filter(Date == data_rel & Municipio %in% drs$Municipio[drs$Regiao == DRS])
tmpM <- deaths_city %>% filter(Date == data_rel & Municipio %in% drs$Municipio[drs$Regiao == DRS])
tmpC_DRS <- cases_DRS %>% filter(Date == data_rel & DRS == params$DRS)
tmpM_DRS <- deaths_DRS %>% filter(Date == data_rel & DRS == params$DRS)
```

# Previsão para dia `r format(params$input$date,"%d/%m/%Y")`

\noindent Projetamos \textcolor{yellow}{`r format(round(tmpC_DRS$Mediana),decimal.mark = ",",big.mark = ".")` [`r format(round(tmpC_DRS$Infimo),decimal.mark = ",",big.mark = ".")`,`r format(round(tmpC_DRS$Sup),decimal.mark = ",",big.mark = ".")`] Casos Confirmados} e \textcolor{red}{`r format(round(tmpC_DRS$Mediana),decimal.mark = ",",big.mark = ".")` [`r format(round(tmpC_DRS$Infimo),decimal.mark = ",",big.mark = ".")`,`r format(round(tmpC_DRS$Sup),decimal.mark = ",",big.mark = ".")`] Mortes} na DRS `r paste(DRS,ifelse(DRS == "Grande São Paulo","(excluindo a Cidade de São Paulo)",""))` em `r format(params$input$date,"%d/%m/%Y")`

\begin{figure}
\centering
\includegraphics[width=\linewidth,height=0.7\textheight]{EPcurve.png}
\end{figure}

# Previsão de Casos por Município em `r format(params$input$date,"%d/%m/%Y")`
\small Apenas para municípios com mais de 100 casos em `r format(params$dmin,"%d/%m/%Y")`
...
```{r,echo=FALSE,warning=FALSE}
peak <- peak[peak$MMediana > 1,]
peak <- peak[order(peak$TMediana,decreasing = F),]
names(peak) <- c("Município","Pico Mín.","Pico Mediana","Pico Máx.","Mortes Mín.","Mortes Mediana",
                 "Mortes Máx.")
kable(peak[,c(1:4)],row.names = F,format = "latex",align = "lccc",digits = 0,
      format.args = list(big.mark = ".")) %>% kableExtra::kable_styling(font_size = 8,latex_options = "basic")
```

# Previsão do Número de Mortes no Dia do Pico por Município

```{r,echo=FALSE,warning=FALSE}
peak <- peak[order(peak[,6],decreasing = T),]
kable(peak[,c(1,5:7)],row.names = F,format = "latex",align = "lccc",digits = 0,
      format.args = list(big.mark = ".")) %>% kableExtra::kable_styling(font_size = 8,latex_options = "basic")
```
...
\noindent Os modelos foram escolhidos dentre dezenas de milhares de modelos candidatos, pois se aderiram bem aos dados observados. A aderência foi mensurada pela diferença entre o número de mortes e o número de casos confirmados nas DRSs com mais de 50 mortes e 500 casos, respectivamente, durante o período de uma semana terminando em `r params$data`
...
\noindent Para mais informações acesse \href{https://www.ime.usp.br/~pedrosp/covid19/}{\textcolor{blue}{https://www.ime.usp.br/~pedrosp/covid19/}}
