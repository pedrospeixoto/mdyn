...
server <- function(input, output) {
  ...
  output$app = renderUI(
    navbarPage(theme = shinytheme("yeti"),"Propensão Relativa ao Risco COVID-19",windowTitle = "Propensão Risco COVID-19",
               ...
               tags$head(tags$link(rel = "shortcut icon", href = "favicon.png"),tags$style("#tbl {white-space: nowrap;}"),
                 ...
                 sidebarLayout(
                   ...
                   mainPanel(
                     ...
                     br(),
                     br(),
               fluidRow(column(12,align = "center",h2("Municípios Relativamente Propensos ao Risco COVID-19"))),
               ...
               HTML("Tendo em vista que os sistemas de saúde ainda não entraram em colapso, e atentando ás particularidades sociodemográficas de cada Município, <strong>podemos considerar a propensão relativa ao risco como uma medida da subnotificação de casos no Município em relação à subnotifcação no Estado</strong>. Se a <strong>PR</strong> do Município for maior do que um, temos evidências de que a subnotificação de casos nele é maior do que no seu Estado e portanto ele está, relativamente ao seu Estado, mais propenso ao risco, entendido aqui como grande prevalência de casos, do que representado pelos dados oficiais."),
               br(),
               br(),
  ...
  title <- tags$div(tag.map.title, HTML(paste("Propensão Relativa ao Risco COVID-19 IME - USP",dataD)))
